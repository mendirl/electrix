...
@Repository
public class ConsommationBruteNationaleRepository implements ClickHouseRepository<ConsommationBruteNationale> {
  private static final Logger log = LoggerFactory.getLogger(ConsommationBruteNationaleRepository.class);
  private final ClickHouseDatabase database;

  public ConsommationBruteNationaleRepository(ClickHouseDatabase database) {
    this.database = database;
  }

  @Override
  public void insert(List<ConsommationBruteNationale> records) throws SQLException {
    database.insert(records, "consommation_brute_nationale", preparer, this::ensureTableExists);
  }

  private final ClickHouseDatabase.StatementPreparer<ConsommationBruteNationale> preparer = new ClickHouseDatabase.StatementPreparer<>() {
    @Override
    public void prepare(PreparedStatement ps, ConsommationBruteNationale record) throws SQLException {
    ...
    public String getInsertSql() {
      return """
        INSERT INTO consommation_brute_nationale (
  ...
  private void ensureTableExists(Connection conn) throws SQLException {
    String createTableSql = """
      CREATE TABLE IF NOT EXISTS consommation_brute_nationale (
    ...
    log.info("Table 'consommation_brute_nationale' ready");
  }
}
